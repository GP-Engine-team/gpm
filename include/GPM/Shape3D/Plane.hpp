...
namespace GPM
{
    class Plane
    {
        ...
        public:
            ...
            {
                return Vec3::dot(point, m_normal) - m_distance;
            }
            ...
            {
                return plane.m_normal.dot(point);
            }
            ...
            bool isOnPlane (const Plane& plane, const Vec3& point)
            {
                ...
                return rst < plane.m_distance + std::numeric_limits<float>::epsilon() && rst > plane.m_distance - std::numeric_limits<float>::epsilon();
            }
            ...
            const float& getDistance ()   const noexcept { return m_distance;}

            void setNormal   (const Vec3& newNormal)   noexcept { m_normal = newNormal.normalized();}
            void setDistance (float newDistance) noexcept { m_distance = newDistance;}
    };
} // !namespace GPM
