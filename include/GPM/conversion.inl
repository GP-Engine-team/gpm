...
{
    return {m.e[0], m.e[1], m.e[2], m.e[4], m.e[5], m.e[6], m.e[8], m.e[9], m.e[10]};
}

inline constexpr Mat4 toMatrix4(const Mat3& m) noexcept
{
    return {m.e[0], m.e[1], m.e[2], .0f, m.e[3], m.e[4], m.e[5], .0f, m.e[6], m.e[7], m.e[8], .0f, .0f, .0f, .0f, 1.f};
}

inline constexpr Mat3 toMatrix3(const Quat& q) noexcept
{
    const f32 x2{q.x * q.x}, y2{q.y * q.y}, z2{q.z * q.z}, xy{q.x * q.y}, yz{q.y * q.z}, xz{q.x * q.z}, wx{q.s * q.x},
        wy{q.s * q.y}, wz{q.s * q.z}, s_2{2.f / q.length2()};

    // Column-major
    return {1.f - s_2 * (y2 + z2), s_2 * (xy + wz), s_2 * (xz - wy), s_2 * (xy - wz),      1.f - s_2 * (x2 + z2),
            s_2 * (yz + wx),       s_2 * (xz + wy), s_2 * (yz - wx), 1.f - s_2 * (x2 + y2)};
}
...
inline Quat toQuaternion(const Mat3& m) noexcept
{
    ...
    else
    {
        ...
        const u8 NEXT[3] = {1u, 2u, 0u}, j = NEXT[i], k = NEXT[j];
...
{
    return {Transform::translation(transfo.position) * toMatrix4(transfo.rotation) * Transform::scaling(transfo.scale)};
}
