...
{
    return {1.f, .0f, .0f, .0f, .0f, 1.f, .0f, .0f, .0f, .0f, 1.f, .0f, t.x, t.y, t.z, 1.f};
}
...
inline Mat4 Transform::rotationX(const f32 angle) noexcept
{
    ...
    return {1.f, .0f, .0f, .0f, .0f, cosAngle, sinAngle, .0f, .0f, -sinAngle, cosAngle, .0f, .0f, .0f, .0f, 1.f};
}
...
inline Mat4 Transform::rotationY(const f32 angle) noexcept
{
    ...
    return {cosAngle, .0f, -sinAngle, .0f, .0f, 1.f, .0f, .0f, sinAngle, .0f, cosAngle, .0f, .0f, .0f, .0f, 1.f};
}
...
inline Mat4 Transform::rotationZ(const f32 angle) noexcept
{
    ...
    return {cosAngle, sinAngle, .0f, .0f, -sinAngle, cosAngle, .0f, .0f, .0f, .0f, 1.f, .0f, .0f, .0f, .0f, 1.f};
}

// Y * X * Z
inline Mat4 Transform::rotation(const Vec3& r) noexcept
{
    const f32 cx{cosf(r.x)}, sx{sinf(r.x)}, cy{cosf(r.y)}, sy{sinf(r.y)}, cz{cosf(r.z)}, sz{sinf(r.z)};
...
{
    const f32 c{cosf(angle)}, s{sinf(angle)}, t{1.f - c}, x{axis.x}, y{axis.y}, z{axis.z};
...
{
    return {
        s.x, .0f, .0f, .0f, .0f, s.y, .0f, .0f, .0f, .0f, s.z, .0f, .0f, .0f, .0f, 1.f,
    };
}
...
inline Mat4 Transform::lookAt(const Vec3& eyePos, const Vec3& targetPos, const Vec3& normalizedUp) noexcept
{
    const Vec4 forward{(eyePos - targetPos).normalized(), .0f}, right{normalizedUp.cross(forward.xyz), .0f},
        up{forward.xyz.cross(right.xyz), .0f};

    return {right, up, forward, eyePos};
}

inline constexpr Mat4 Transform::symFrustrum(const f32 right, const f32 top, const f32 near, const f32 far) noexcept
{
    const f32 depthInv{1.f / (far - near)};

    return {near / right,            .0f,            .0f,            .0f,
            .0f,            near / top,            .0f,            .0f,
            .0f,            .0f,            -(far + near) * depthInv,            -1.f,
            .0f,            .0f,            -2.f * far * near * depthInv,            .0f};
}

inline Mat4 Transform::perspective(const f32 fovY, const f32 aspect, const f32 near, const f32 far) noexcept
{
...
inline constexpr Mat4 Transform::orthographic(const f32 right, const f32 top, const f32 near, const f32 far) noexcept
{
...
inline constexpr Mat4 Transform::viewport(const f32 x, const f32 y, const f32 width, const f32 height) noexcept
{
    const f32 halfWidth{width * .5f}, halfHeight{height * .5f};

    return {halfWidth, .0f, .0f, .0f, .0f,           halfHeight,     .0f, .0f,
            .0f,       .0f, .5f, .0f, x + halfWidth, y + halfHeight, .5f, 1.f};
}
...
inline constexpr Transform::Transform(const Mat4& m) noexcept : model{m}
{
}
...
inline Mat4 Transform::rotation() const noexcept
{
    ...
    return {{model.c[0].xyz * invScale.x, .0f},
...
inline Vec3 Transform::eulerAngles() const noexcept
{
    if (model.e[9] < 1.f)
    {
        if (model.e[9] > -1.f)
        {
            return {asinf(-model.e[9]), atan2f(model.e[8], model.e[10]), atan2f(model.e[1], model.e[5])};
        }
